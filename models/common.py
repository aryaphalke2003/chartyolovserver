...
class CrossConv2(nn.Module):
    # Cross Convolution Downsample
    def __init__(self, c1, c2, k=3, s=1, g=1, e=1.0, shortcut=False):
        # ch_in, ch_out, kernel, stride, groups, expansion, shortcut
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = CBAM(c1, c_, (1, k), (1, s))
        self.cv2 = CBAM(c_, c2, (k, 1), (s, 1), g=g)
        self.add = shortcut and c1 == c2
...
class C3CBAM(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(CBAM(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)))       
...
class C3x2(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(CrossConv2(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)))
...
class ASPP_5(nn.Module):
    def __init__(self, in_channel=512, out_channel=256):
        super(ASPP_5, self).__init__()
        self.mean = nn.AdaptiveAvgPool2d((1, 1))  # (1,1)means ouput_dim
        self.conv = nn.Conv2d(in_channel,out_channel, 1, 1)
        self.atrous_block1 = nn.Conv2d(in_channel, out_channel, 1, 1)
        self.atrous_block3 = nn.Conv2d(in_channel, out_channel, 3, 1, padding=3, dilation=3)
        self.atrous_block6 = nn.Conv2d(in_channel, out_channel, 3, 1, padding=4, dilation=4)
        self.atrous_block12 = nn.Conv2d(in_channel, out_channel, 3, 1, padding=6, dilation=6)
        self.atrous_block18 = nn.Conv2d(in_channel, out_channel, 3, 1, padding=8, dilation=8)
        self.atrous_block24 = nn.Conv2d(in_channel, out_channel, 3, 1, padding=12, dilation=12)
        self.conv_1x1_output = nn.Conv2d(out_channel * 7, out_channel, 1, 1)
    ...
    def forward(self, x):
        ...
        net = self.conv_1x1_output(torch.cat([image_features, atrous_block1, atrous_block3,
                                              atrous_block6, atrous_block12, atrous_block18, atrous_block24], dim=1))
        return net 
...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        atrous_block12 = self.atrous_block12(x)
        net = self.conv_1x1_output(torch.cat([image_features, atrous_block1, atrous_block3,
                                              atrous_block6, atrous_block12], dim=1))
        return net    
